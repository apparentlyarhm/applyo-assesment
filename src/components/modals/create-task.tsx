...
import { Board, Task, useUserData } from "@/hooks/useUserData";
...
interface CreateTaskDialogProps {
    email: string;
    children: React.ReactNode; // anything can be a trigger
...
interface EditTaskDialogProps {
    task: Task;
    boardId: string; 
    children: React.ReactNode; // same here
}

export function CreateTaskDialog({ email, currentBoard, children }: CreateTaskDialogProps) {
...
// we just copy the above and make necessary changes. props wise it just takes 
// the task object to pre fill the form and re-uses the `editTask` method
export function EditTaskDialog({ task, boardId, children }: EditTaskDialogProps) {
    ...
    // This effect ensures that if the dialog is opened for a different task,
    // the form fields reset to the new task's data.
    useEffect(() => {
        if (isOpen) {
            setTitle(task.title);
            ...
            // Reset the date from the original task prop every time
    ...
    const handleSubmit = () => {
        // Create the 'updates' object. It only needs the fields that changed.
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className={clsx("fixed top-1/2 left-1/2 w-[95%] max-w-lg -translate-x-1/2 -translate-y-1/2 bg-white p-6 sm:p-8 rounded-xl shadow-2xl", nunito.className)}>
                    ...
                    <div className="mt-8 flex justify-end gap-3">
                        ...
                        <button
                            onClick={handleSubmit} // this handleSubmit is the scope of this function.
                            className="px-5 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:bg-blue-300 disabled:cursor-not-allowed"
                            disabled={!title.trim()}
                        >
                            Update
